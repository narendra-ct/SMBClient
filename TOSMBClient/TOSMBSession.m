...
@property (nonatomic, strong) NSOperationQueue *uploadsQueue; /* Operation queue for file uploads. */

@property (nonatomic, strong, readwrite) NSArray *downloadTasks;

@property (nonatomic, strong, readwrite) NSArray *uploadTasks;
...
- (SMBSessionUploadTask *)uploadTaskForFileAtPath:(NSString *)path destinationPath:(NSString *)destinationPath delegate:(id<TOSMBSessionUploadTaskDelegate>)delegate
{
    [self setupUploadQueue];
    
    SMBSessionUploadTask *task = [[SMBSessionUploadTask alloc] initWithSession:self filePath:path destinationPath:destinationPath delegate:delegate];
    self.uploadTasks = [self.uploadTasks ? : @[] arrayByAddingObjectsFromArray:@[task]];
    return task;
}
...
{
    if (self.uploadsQueue)
        return;
    
    self.uploadsQueue = [[NSOperationQueue alloc] init];
    self.uploadsQueue.maxConcurrentOperationCount = self.maxDownloadOperationCount;
}
